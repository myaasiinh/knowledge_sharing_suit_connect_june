  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: PageView.builder(
        ...
        onPageChanged: (page) {
          setState(() {_slider.currentPage = page;});
        },
        itemCount: lottieAnimationPaths.length,
        itemBuilder: (_, i) => Center(
          child: Lottie.asset(lottieAnimationPaths[i],repeat: true,reverse: false,
          ),
        ),
      ),
      bottomNavigationBar: BottomNavigationBar(currentIndex: _slider.currentPage,
        onTap: (page) {setState(() {});_slider.jumpTo(page);
        },
        items: const [ BottomNavigationBarItem(icon: Icon(Icons.animation_outlined), label: "Animation 1"),
          BottomNavigationBarItem(icon: Icon(Icons.brightness_medium), label: "Animation 2"),
          BottomNavigationBarItem(icon: Icon(Icons.car_rental), label: "Animation 3")
        ],
      ),
    );
  }
